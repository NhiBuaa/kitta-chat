...
| `message_boundary_pending` | 2 | M1/M2 assigned to dedicated follow-up; route-specific limiter blocked; no stable Redis class/actor contract |
...
| `auth_recovery` — 2 HTTP | `POST /api/auth/forgot-password`, `/reset-password/:id/:token` | Mandatory canonical network actor bucket | Target-wide HMAC account bucket pending; raw email, user ID, or token cannot be actor | Fail closed | Both consume account-recovery capacity; splitting lets an attacker alternate email-queue requests and reset verification/hash work | Forgot-password abuse could consume a shared budget needed for legitimate reset completion; request/commit separation remains an assignment ambiguity |
...
| `message_boundary_pending` — 2 HTTP, planning-only | `POST /api/messages`; `GET /api/messages/:userId1/:userId2`; both assigned to dedicated follow-up | Canonical network actor is planning-safe only; caller-supplied sender, `userId1`, receiver, group or conversation ID is never actor | No user or target-wide bucket approved; temporary network-only enforcement requires a separate maintainer decision and migration/removal contract | Route-specific distributed application limiter implementation blocked by default | A temporary shared network defense could limit unauthenticated route rotation but cannot substitute for access control | Reclassify after the combined follow-up produces verified-principal/authz semantics; do not fossilize the missing-auth state in Redis keys |
...
- `POST /api/messages` and `GET /api/messages/:userId1/:userId2`: M1/M2 are assigned to `dedicated message-access-control follow-up required / identifier pending`. Planning-only `message_boundary_pending` remains blocked for route-specific implementation; caller-supplied identities cannot be used as actor keys, and temporary network-only application limiting is not approved.
