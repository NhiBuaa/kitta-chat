        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            <div className="absolute right-0 top-0 bottom-0 w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col">
                ...
                <div className="h-32 bg-blue-600 relative flex items-center justify-center">
                    {/* Nút đóng */}
                    <button onClick={onClose} className="absolute top-4 right-4 text-white hover:text-gray-200 text-2xl font-bold focus:outline-none"> &times; </button>
                    ...
                    <div className="absolute -bottom-10 relative group">
                        ...
                        <label htmlFor="upload-avatar" className="absolute inset-0 bg-black/30 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                ...
                <div className="mt-4 px-6 py-4 flex-grow overflow-y-auto">
                    <h2 className="text-xl font-bold text-center text-gray-800">{user.displayName}</h2>
                    <p className={`text-center text-sm mb-6 font-medium ${formData.isOnline ? 'text-blue-600' : 'text-gray-500'}`}>
                        {formData.isOnline ? 'Đang hoạt động' : 'Đang tắt trạng thái'}
                    </p>
