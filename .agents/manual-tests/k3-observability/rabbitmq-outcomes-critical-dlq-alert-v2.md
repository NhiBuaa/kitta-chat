...
- Guide revision: v2 (locked)
- Approved by: user
- Approved at: 2026-08-06T11:47:01+07:00
- Revision note: v2 changes only the approved execution runtime from Node 22 to Node 24; Test Cases and expected observability behavior are unchanged. Revision v1 remains immutable.

## Prerequisites

- Environment: the dedicated repository worktree with Node 24 and server dependencies installed; this execution uses Node `v24.18.0` and the existing `server` Node test setup.
...
This guide is locked at revision v2 after explicit human approval. Do not rewrite it from later observations. Revision v1 remains immutable. Execute each run as a separate JSONL Evaluation record at `.agents/manual-tests/k3-observability/rabbitmq-outcomes-critical-dlq-alert-v2.evaluations.jsonl`.
