...
- Guide revision: `k4-issue-85-r3`
- Supersedes: `k4-issue-85-r2` (immutable draft; never rewrite)
- Status: LOCKED — human-approved; immutable
- Approved by: user
- Approved at: 2026-08-14T04:37:23Z
- Evaluation history (after approval): `.agents/manual-tests/k4-performance-evidence/issue-85-provenance-report-validator-r3.evaluations.jsonl`
...
- Exact-byte boundary gate: by default, the independent verifier hashes SHA-256 over the whole exact persisted bytes of the source-inventory artifact on disk. It does not parse, reserialize, canonicalize, normalize, or require a separate authority/schema to define that whole-file byte boundary. Only when the contract deliberately uses a representation other than whole-file bytes or a non-self-referential boundary must the implementation cite an authority/schema that defines it; a locally invented alternate boundary or canonicalization is `BLOCKED`.

## Locked Test Cases

### TC-85-01: Build a reproducible source inventory from whole-file exact retained bytes

- Purpose: Verify that finalized source artifacts and the environment manifest produce an auditable source inventory and reproducible `source_inventory_sha256` by default over the whole exact persisted source-inventory artifact bytes, without allowing an implementation-only alternate boundary.
...
- Steps:
  ...
  2. Locate the declared `source-inventory`, enumerate every source artifact it names, and confirm that the default persisted representation is the complete artifact bytes on disk. If the contract declares an alternate representation or a non-self-referential boundary, record the specific authority/schema name and version that defines it.
  ...
  4. Independently hash the complete exact retained source-inventory artifact bytes on disk with SHA-256. Do not parse, reserialize, canonicalize, normalize, or accept an implementation-only alternate boundary. Compare the independent digest with `source_inventory_sha256`. If and only if an authority/schema explicitly defines an alternate representation or non-self-referential boundary, verify that declared representation instead and capture its reference.
...
- Expected results:
  ...
  - The SHA-256 of the complete exact retained source-inventory artifact bytes on disk equals the recorded `source_inventory_sha256`, and repeating verification on unchanged bytes produces the same inventory and digest.
  - Whole-file bytes require no separate authority/schema to define their boundary. If the contract uses an alternate representation or non-self-referential boundary, that authority/schema is cited and governs the verification; a locally chosen serialization, canonicalization, or alternate boundary without such authority makes this case `BLOCKED`.
...
- Evidence to capture:
  - Complete exact retained source-inventory artifact bytes, recorded digest, and the independent SHA-256 transcript; include an authority/schema name/version/reference only when an explicitly declared alternate or non-self-referential representation applies.
  - Inventory entry listing with paths, types, byte sizes, and SHA-256 values.
  - Independent exact-byte hashing command/output for source files and the whole source-inventory artifact, with no parsing or reserialization.
