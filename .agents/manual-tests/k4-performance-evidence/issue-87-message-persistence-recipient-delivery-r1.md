...
- Guide revision: `k4-issue-87-r1`
- Guide status: DRAFT — human approval is required before execution or implementation handoff
- Approved by: PENDING
- Approved at: PENDING
...
- Purpose: Verify that a successful latency sample requires the existing Socket.IO contracts plus one unique correlation identity: idempotency key, acknowledgement, message ID, sender, recipient, and legacy `Message.conversationId`.
- Steps:
  ...
  2. Select one opportunity and trace its unique `idempotencyKey` through runner output, sender/acknowledgement/receiver attribution records, and the recipient `getMessage` record.
  3. Compare the acknowledgement `{ success, realId }` with the delivered message `_id`, sender identity, recipient identity, and legacy public `Message.conversationId`.
  4. Inspect the persisted raw evidence and determine whether the opportunity is included in the successful delivery set and in any cross-replica qualification input.
- Expected results:
  - The `sendMessage` callback returns `{ success: true, realId }`; its `realId` equals the delivered payload `_id`.
...
- Evidence to capture:
  - Redacted runner opportunity record and the four correlated raw records.
  - The acknowledgement `realId`, delivered `_id`, actor identities, and legacy conversation identifier comparison.
...
- Purpose: Verify that a multi-replica message claim is based on measured attribution, not topology inventory alone.
- Steps:
  ...
  4. Inspect raw nginx/backend sources, parser diagnostics, source digests, and the resulting correlation qualification.
  5. If the sender and recipient were observed on the same replica, or if the source window is incomplete, verify the corresponding non-claim outcome instead of inferring cross-replica delivery from the configured topology.
- Expected results:
  ...
  - Same-replica evidence is marked `TOPOLOGY_NOT_EXERCISED`; ambiguous, truncated, or incomplete source evidence is marked `OBSERVATION_INCOMPLETE` for the affected claim.
...
- Evidence to capture:
  ...
  - The correlated sender/recipient replica identities and the final claim-eligibility flags.
...
This guide becomes immutable after explicit human approval. Create `k4-issue-87-r2` for any semantic change; never rewrite this revision to fit later observations. Store each execution as a separate append-only Evaluation JSONL record.
