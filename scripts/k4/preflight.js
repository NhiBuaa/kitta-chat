...
function setupPreflightCommands(plan) {
  ...
  return [
    ...
    { prerequisite: "login", args: [...compose, "exec", "-T", "runner", "node", "-e", "fetch('http://nginx/api/auth/login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: process.env.K4_BENCHMARK_EMAIL, password: process.env.K4_BENCHMARK_PASSWORD }) }).then(async (r) => { if (!r.ok) process.exit(1); const body = await r.json(); process.stdout.write(JSON.stringify({ token: body.token, user: body.user })); })"] },
